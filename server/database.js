...
ACCOUNTS = {
    ...
    changePassword: async function(username, oldpassword, password) {
        var status = await updatePassword(username, oldpassword, password);
        if (status) {
            return true;
        }
        return false;
    ...
    deleteAccount: async function(username, password) {
        var status = await deleteCredentials(username, password);
        return status;
    },
...
async function deleteCredentials(username, password) {
    var cred = await getCredentials(username);
    if (cred) {
        if (bcrypt.compareSync(password, cred.password)) {
            try {
            ...
            warn('WARNING: Unauthorized attempt to delete account!');
            return 1;
        }
    }
    return 2;
};
async function updatePassword(username, oldpassword, password) {
    var cred = await getCredentials(username);
    if (cred) {
        if (bcrypt.compareSync(oldpassword, cred.password)) {
            ...
            warn('WARNING: Unauthorized attempt to change password!');
        }
    }
    return false;
};
